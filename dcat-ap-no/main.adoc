...
In this showroom we have a link:files/demo-datasets-en#demoDataset1[demo dataset, called "Demo controlled vocabulary 1"] (in link:files/demo-datasets.ttl[RDF Turtle]):
...
** one link:files/demo-datasets-en#demoDistribution1[machine-readable] (https://data.norge.no/specification/dcat-ap-no#Distribusjon-format[format (dct:format) &#x29C9;, window="_blank", role="ext-link"] = RDF_TURTLE), 
** one link:files/demo-datasets-nb#demoDistribution2[human-readable in Norwegian bokmål] (format = HTML), and 
** another link:files/demo-datasets-en#demoDistribution3[human-readable in English] (format = HTML).
